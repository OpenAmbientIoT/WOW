...
<template>
  ...
  <div :class="'wavelet wavelet_packet' + (wavelet.options.fadein ? ' wavelet_fadein' : '' )
    ...
    <svg v-if="!wavelet.options.ringsFadeout" :key="Date.now()" xmlns="http://www.w3.org/2000/svg" :style="('width: ' + wavelet.size + 'px;' + 'height: ' + wavelet.size + 'px;')">
      <circle r="10" stroke="#fff" :stroke-width="2 * (wavelet.size/64)" fill="none" cx="50%" cy="50%">
        ...
        <animate attributeType="CSS" attributeName="opacity" values="0;1;0" dur=".4" begin="0" repeatCount="1" fill="freeze"/>
      </circle>
    </svg>
